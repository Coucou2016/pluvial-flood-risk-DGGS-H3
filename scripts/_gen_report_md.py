...
def main() -> None:
    ...
    ada = d.get("adaptive_r11_hotspot_retention") or d["adaptive"]["rows"][0]
    if isinstance(ada, dict) and "n_adaptive_mixed" in ada:
        ada_n_coarse = ada["n_coarse"]
        ada_n_adaptive = ada["n_adaptive_mixed"]
        ada_n_uniform = ada["n_uniform_fine"]
        ada_ratio = float(ada["cell_count_ratio_vs_uniform"])
        ada_refined = ada["n_parents_refined"]
        ada_recall = ada.get("hotspot_recall")
    else:
        ada_n_coarse = int(float(ada["adaptive_n_coarse"]))
        ada_n_adaptive = int(float(ada["adaptive_n_adaptive"]))
        ada_n_uniform = 12838
        ada_ratio = float(ada.get("adaptive_cell_count_ratio", ada_n_adaptive / ada_n_uniform))
        ada_refined = int(float(ada["adaptive_n_parents_refined"]))
        ada_recall = None
    neg = d["negative_control"]
    land = d.get("land_mask_sensitivity", {})
    abl = d.get("source_ablation", {})
    oof = d.get("oof_extended_metrics", {}).get("lower_manhattan", {}).get("pooled", {})
...
在 H3 R9 上组装 **n_cells = 262**（bbox `[-74.02, 40.70, -73.97, 40.76]`）。主评价为 **spatial H3-block CV**（12 个 R7 块，5 折）：准确率 **{f3(scv['spatial_cv_accuracy_mean'])} ± {f3(scv['spatial_cv_accuracy_std'])}**，F1 **{f3(scv['spatial_cv_f1_mean'])}**；正类占比 **{lm['positive_prevalence']*100:.1f}%**；恒判正基线 accuracy **{f3(b['always_positive_mean_acc'])}**、F1 **{f3(b['always_positive_mean_f1'])}**，模型超过该基线。留出 pooled ROC-AUC **{f3(scv['spatial_cv_roc_auc_pooled'])}**、AP **{f3(scv['spatial_cv_pr_auc_pooled'])}**。扩展试点 n=956：acc **{f3(ex['spatial_cv']['spatial_cv_accuracy_mean'])} ± {f3(ex['spatial_cv']['spatial_cv_accuracy_std'])}**，F1 **{f3(ex['spatial_cv']['spatial_cv_f1_mean'])}**，pooled ROC-AUC **{f3(ex['spatial_cv']['spatial_cv_roc_auc_pooled'])}**。尺度损失（strict area budget + study_domain_mask）：R10→R9 mean soft Jaccard **0.220**，R10→R8 mean **0.136**（n_fine=1788，n_coarse R9=262）。自适应相对均匀 R11 单元数比 **≈{ada_ratio:.3f}**（{ada_refined}/262 父单元加密 → {ada_n_adaptive:,} vs {ada_n_uniform:,}；Option A R11 重提特征 hotspot recall = {ada_recall if ada_recall is not None else '见 registry'}）。FloodNet 为严格留出诊断（LM ROC-AUC {f3(fn['roc_auc'])}，{fn['n_study_cells_with_sensor']} 传感器单元）。
...
**如何读：** 先看正类占比（63.7%），再看恒判正基线（{f3(b['always_positive_mean_acc'])} / {f3(b['always_positive_mean_f1'])}），最后才看模型（{f3(scv['spatial_cv_accuracy_mean'])} / {f3(scv['spatial_cv_f1_mean'])}）。ROC-AUC/AP 是阈值无关排序指标；Table 3 Panel A 还报 MCC/bal.acc 等（registry `oof_extended_metrics`）。  
...
|---|---|
| Fixed R9 | {ada_n_coarse} |
| Adaptive R9/R11 | {ada_n_adaptive} |
| Uniform R11 | {ada_n_uniform} |

筛选：分数 ≥0.8 分位或不确定区间，再扩一环；**deployment_full** 入样分数。Option A：R11 重提特征后 scorable children 上 hotspot recall = {ada_recall if ada_recall is not None else 'n/a'}。

**来龙去脉：** 自适应主叙事是表示密度；Option A 另做真 R11 重提特征诊断（DEM 边 NaN 丢弃后评分）。  
**如何读：** {ada_n_adaptive:,} / {ada_n_uniform:,} ≈ {ada_ratio:.3f}（{ada_refined}/262 父单元加密）。  
...
|--------|--------|---------|
| DEP-only | {f3(abl['pilots']['lower_manhattan'][0]['roc_auc_pooled'])} | {f3(abl['pilots']['manhattan_expanded'][0]['roc_auc_pooled'])} |
| 311-only | {f3(abl['pilots']['lower_manhattan'][1]['roc_auc_pooled'])} | {f3(abl['pilots']['manhattan_expanded'][1]['roc_auc_pooled'])} |
| Composite | {f3(abl['pilots']['lower_manhattan'][4]['roc_auc_pooled'])} | {f3(abl['pilots']['manhattan_expanded'][4]['roc_auc_pooled'])} |
| 311 w/o building_density | {f3(abl['pilots']['lower_manhattan'][6]['roc_auc_pooled'])} | {f3(abl['pilots']['manhattan_expanded'][6]['roc_auc_pooled'])} |
...
|------|---------|----------------|-----|-----|
| all_cells | {land['rows'][0]['n_cells']} | {f3(land['rows'][0]['roc_auc_pooled'])} | {f3(land['rows'][0]['accuracy_mean'])} | {f3(land['rows'][0]['f1_mean'])} |
| land_frac ≥ 0.5 | {land['rows'][1]['n_cells']} | {f3(land['rows'][1]['roc_auc_pooled'])} | {f3(land['rows'][1]['accuracy_mean'])} | {f3(land['rows'][1]['f1_mean'])} |
| centroid_on_land | {land['rows'][2]['n_cells']} | {f3(land['rows'][2]['roc_auc_pooled'])} | {f3(land['rows'][2]['accuracy_mean'])} | {f3(land['rows'][2]['f1_mean'])} |
...
扩展 OOF 阈值指标（LM pooled）：MCC {f3(oof.get('mcc', 0))}，balanced accuracy {f3(oof.get('balanced_accuracy', 0))}，specificity {f3(oof.get('specificity', 0))}。
...
*Generated for Option B freeze · registry {d['generated_utc']} · fail_closed={d['fail_closed']} · seed={d['random_seed']} · freeze_tag={d.get('freeze_tag','n/a')}*
"""

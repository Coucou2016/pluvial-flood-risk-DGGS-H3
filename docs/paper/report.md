...
*Generated for Option B freeze · registry 2026-09-21T02:51:33.767437+00:00 · fail_closed=True · seed=42*

---

## 9. Construct-validity revision (2026-09-21) — 来龙去脉

本轮按审稿 §二十三优先序落地，权威数字仍只认 `outputs/paper_results.json`（Option B n=262 / 扩展 n=956，**不回退 141**）。

1. **语义 + 源消融：** 标签改为 evidence-positive / evidence-unrecorded；新增 311−building_density（LM pooled AUC 0.837，仍 >0.5；复合非 DEP 独占）。
2. **块敏感性 + LOBO + Moran：** LM LOBO pooled AUC 0.858；残差 Moran I 0.047（目标 0.388）——阻断降低但未消灭空间泄漏。
3. **尺度损失域掩膜：** R10→R9 统一为 modelling 262（n_fine=1788）；mean Jaccard R9=0.220 / R8=0.136；max 不作“最优策略”。
4. **Sandy：** 仍用 OOF 分数；排除 2012-10-27–11-05 的 311（9 条）仅翻 1 格，结论不变。
5. **自适应：** 降为 Supplement 表示规模实验（仅 cell count）。
6. **降雨：** 主主张改为 `S_h(c)=f_θ(X_c)`；`S_h(c,r)` 留作未来。

硬门禁：`tests/test_major_revision_gates.py`；pytest 89 passed / 1 skipped。
